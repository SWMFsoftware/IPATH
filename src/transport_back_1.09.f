...
c=======================================================================
c               Read in outputs from the acceleration module
c=======================================================================

c------   Read in the distribution function within shock complex  ------
       
       open(21, file="./dist_at_shock.dat", form="formatted")
       line = 0
       do
     ...
     1          **2.0) - mp*co**2.0 )/ eo
       ...
       do i = 1, line         
          dist_fp(ishl, ip, iphi) = dist_param(2, i)
          dist_fp_hi(ishl, ip, iphi) = dist_param(3, i)
          ip = ip + 1
...
c------  Read in the maximum energy at each time and longitude ----
       open(22, file="./shock_momenta.dat", form="formatted")
       ...
       if ( (phi_e .gt. phi_0(phi_no)) .or. (phi_e .lt. phi_0(1)) ) then
            phi_e_index = floor(phi_no/2.)
       endif 
...
c------   Read in distribution functions at the downstream -------- 
       open(24, file="./esc_distr_dn.dat", form="formatted")
...
c------    Read in the boundaries of all shells   ---------------------

       open(25, file="./all_shell_bndy.dat", form="formatted")
...
c------   calculate Kappa_perp at 1AU numerically  ---------------------
       do j = 1, p_num
...
!     13 -> state variable upon crossing inner boundary {energy, time,
!           momentum, pitch} 
...
c-----   generate random iseed based on rank  -------------------------- 
...
c------  Time loop  ----------------------------------------------------
       do itime = 1, t_num ! in V1.05 we only consider up to 1AU
          ...
          time_fp = time_start + itime *(time_end -time_start)/t_num
...
c             print*,"miu0 is: ", miu0
             k = 1              ! k is number of steps. 
                            ![since dt can vary, this is not fully correct]
...
10         continue        
!     use if ... continue to be safe for fortran 77 compilers
...
c     1         (dabs(tnew) .le. (time_fp - time_start)*t_o)) then
     ...
     1          shell_time(1))*t_o) ) then
                ! check if it's within the step limit 
...
c=======================================================================
!        First updating (r, miu, p) using RK4 method
!        this is for adiabatic cooling, pitch angle focusing,
!        and particles moving along the field line 
c-----------------------------------------------------------------------
c              print*, r, miu, p
...
c=======================================================================
!        second, additional dmiu due to turbulence scattering
...
c=======================================================================
!         Third: adding perpendicular diffusion.
!         dX is the perpendicular displacement
!         then update r and phi 
c-----------------------------------------------------------------------
c              print*, '1. phinew, rnew', phinew, rnew/AU
c              Kappa_Perp = Kappa_Perp0(i) *(rnew/AU)**1.2
c              print*,'simple    Kperp:',Kappa_perp,     rnew/AU

              B_r = parker_field(rnew)   
c                        ! introduce this parameter to optimize speed
     ...
     1             B_r))**(7./9.)*(AU/rnew)**(7./6.)* Kappa_Perp1(i)
              ! changed in V1.05
              ! calculate local Kappa_perp using its v, B, r 
              ! dependences. 
...
c~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
c       Now done updating quasi-particles' status
c       Start checking if it hits the boundary
c~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
c-----------------------------------------------------------------------
c------ Get the shock front radius at this moment and longitude  -------
...
c-----------------------------------------------------------------------
              goto 666  ! currently skipping this part
...
c-----------------------------------------------------------------------
!
!      Check if particle encounters the shock  
...
15         continue         ! record particle, adding to ensemble 
...
20         continue ! now onto the next test particle
         enddo  ! for j loop (test particles)
     ...
     1                      , phirec(j), rrec(j)
       ...
       enddo  ! for i loop (momentum)
       enddo  ! for itime loop (time)
...
c========== Parker Field ===============================================
!
!       calculate total B at r according to parker spiral
!
c-----------------------------------------------------------------------
...
!
!    The following function gives the inverse of error function
!    define through erf(x) = 2/sqrt(pi) \int_0^x dt exp(-t^2).
!
c---------------------------------------------------------------------
...
!
!      The purpose for this function is to avoid repeating calculation 
!      for the normalization factor for different K at same R
!
c-----------------------------------------------------------------------
...
c      s2d    = 0.8   ! b_2d^2/b^2 
      ...
      gyrof  = eo * parker_field(r) /mp/gamma0
...
!
!       interpolate the shock location at time T
!
c-----------------------------------------------------------------------
...
c=============  get fp at time T  ======================================
!
!       interpolate the fp at given location at time T
!       
c-----------------------------------------------------------------------
